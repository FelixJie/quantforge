...
# 并行上限：同一时间最多几个产业链精读在跑（全量精读吃 LLM+CPU，过多会拖垮后端）。
# 可经 QF_RESEARCH_MAX_CONCURRENT 覆盖。
_RESEARCH_MAX_CONCURRENT = int(os.getenv("QF_RESEARCH_MAX_CONCURRENT") or 3)
        ...
        for attempt in range(2):
            ...
            try:
                txt = await chat(system=_MAP_PROMPT, user=user,
                                 max_tokens=max_tokens + attempt * 2048, caller="industry_map",
                                 provider=provider)
...
async def _run_keyword_analysis(keyword: str, read_limit: int = 0,
    ...
    try:
        ...
        try:
            ...
            part_a, part_b, part_c, part_d, part_e, part_f, part_g, part_h = await _cancellable_gather(
                slug,
                _chat_json(_REDUCE_A_PROMPT, reduce_user, 16000, "industry_research", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_B_PROMPT, reduce_user, 8000, "industry_research", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_C_PROMPT, reduce_user, 8000, "industry_decision", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_D_PROMPT, reduce_user, 8000, "industry_research", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_E_PROMPT, reduce_user, 6000, "industry_research", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_F_PROMPT, reduce_user, 6000, "industry_research", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_G_PROMPT, reduce_user, 8000, "industry_research", provider=_rp, timeout=_reduce_timeout),
                _chat_json(_REDUCE_H_PROMPT, reduce_user, 6000, "industry_research", provider=_rp, timeout=_reduce_timeout),
            ...
            if not result:
                raise RuntimeError(
                    ...
                    f"E={part_e}; F={part_f}; G={part_g}; H={part_h}")
        except Exception as e:
...
_DAILY_KEYWORDS_CFG_KEY = "research:daily_keywords"
...
def _get_daily_keywords() -> list[str]:
    """读取每日定时关键词清单（持久化于 app_config，缺省回退默认种子）。"""
    from quantforge.data.storage import db_cache
...
def _set_daily_keywords(keywords: list[str]) -> list[str]:
    """覆盖写入每日定时关键词清单，返回规整后的清单。"""
    from quantforge.data.storage import db_cache
    ...
    if out:
        ...
        # 列表清空时删掉 key，使 _get_daily_keywords() 的 None 分支生效，默认种子不丢失
        db_cache.app_config_delete(_DAILY_KEYWORDS_CFG_KEY)
    return out


# ── 关键词逐小时错开排期 ─────────────────────────────────────────────────────
# 需求：多个每日关键词不要在 0 点同时开跑（同时跑会互相抢线程池/LLM 配额、
# 把后端拖垮），改成按清单顺序逐个错开——第 1 个 0 点、第 2 个 1 点、第 3 个 2 点…
# 每次只唤醒跑一个，天然串行、绝不并发。
_DAILY_START_HOUR = int(os.getenv("QF_RESEARCH_START_HOUR") or 0)             # 首个关键词运行整点（默认 0 点）
_DAILY_STAGGER_HOURS = max(1, int(os.getenv("QF_RESEARCH_STAGGER_HOURS") or 1))  # 关键词之间错开的小时数
# 记录每个关键词「今天已尝试运行」的日期（slug → YYYY-MM-DD）。失败时 generated_at
# 不会更新，靠它避免在当天反复紧凑重试拖垮后端；跨天自然失效，次日按时重跑。
_DAILY_ATTEMPTED: dict[str, str] = {}


def _kw_run_hour(idx: int) -> int:
    """清单中第 idx 个关键词（从 0 计）的预定运行整点（0~23）。"""
    return (_DAILY_START_HOUR + idx * _DAILY_STAGGER_HOURS) % 24
...
def _next_due_keyword() -> tuple[str, float] | None:
    ...
    规则：第 i 个关键词预定在 ``_kw_run_hour(i)`` 点运行——
      - 该时刻今天已过且今天还没跑过 → 立即补跑（返回极小等待）；
      - 否则取「下一次预定时刻」最近的关键词。
    ...
    for idx, kw in enumerate(kws):
        target = now.replace(hour=_kw_run_hour(idx), minute=0, second=0, microsecond=0)
        if target <= now:
            if not _kw_done_today(kw, today):
                return (kw, 1.0)            # 错过且今天没跑 → 立即补
            target += timedelta(days=1)     # 今天已跑 → 顺延到明天该时刻
        wait = (target - now).total_seconds()
        if best is None or wait < best[1]:
            best = (kw, wait)
    if best is None:
        return None
    return (best[0], max(1.0, best[1]))


async def daily_keyword_scheduler() -> None:
    """后台循环：按「逐小时错开」的时刻表串行重跑每日关键词。

    第 1 个关键词 0 点跑、第 2 个 1 点、第 3 个 2 点…（起始时/间隔可由
    QF_RESEARCH_START_HOUR / QF_RESEARCH_STAGGER_HOURS 调整）。每次只唤醒跑一个，
    绝不并发。启动时对「今天尚未生成且预定时刻已过」的关键词立即补跑，保证页面有
    最新内容；之后各自对齐到每天的预定整点。
    """
    import asyncio
    logger.info(
        f"research.daily_keyword_scheduler: starting "
        f"(start_hour={_DAILY_START_HOUR}, stagger={_DAILY_STAGGER_HOURS}h, "
